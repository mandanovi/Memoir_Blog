...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        if current_user.is_anonymous:
            return abort(403, description="User is anonymous. You are forbidden to do this act.")
        elif current_user.id != 1:
            return abort(403, description="User is not an admin. You are forbidden to do this act.")
        else:
...
def register():
    ...
    if form.validate_on_submit():
        pw_hash = generate_password_hash(password=form.password.data, method='pbkdf2:sha256', salt_length=8)
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            ...
            return redirect(url_for('login'))
        new_comment = Comment(author_id=current_user.id, post_id=post_id, text=form.body.data)
        db.session.add(new_comment)
        db.session.commit()
        return redirect(url_for('show_post', post_id=post_id))
    form.body.data = ""
    return render_template("post.html", form=form, post=requested_post, gravatar=gravatar, logged_in=current_user.is_authenticated)
